...
func (r *Runtime) runTask(ctx context.Context) {
	...
	for stepIndex, step := range steps {
		completed, newSummary := r.executeStep(ctx, currentWorker, taskID, stepIndex, step, maxIterations, summary, plan, steps)
		if !completed {
			log.Printf("❌ Step %d could not be completed, aborting task execution.\n", stepIndex+1)
			return
		}
		summary = newSummary
	}
